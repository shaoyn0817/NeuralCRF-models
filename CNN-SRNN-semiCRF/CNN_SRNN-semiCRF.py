...
class Atten_SRNN_semiCRF(nn.Module):
    ...
    def __init__(self, vocab_size, tag_to_ix, embedding_dim, hidden_dim, max_span_len, word_to_ix, word_vecs, char_to_ix, char_vecs, char_embedding_dim):
        super(Atten_SRNN_semiCRF, self).__init__()
        ...
        self.conv2 = nn.Sequential(  
                     nn.Conv2d(in_channels=1, out_channels=34, kernel_size=(self.char_embedding_dim, 2),  
        ...
        self.conv3 = nn.Sequential(  
                     nn.Conv2d(in_channels=1, out_channels=33, kernel_size=(self.char_embedding_dim, 3),  
        ...
        self.conv4 = nn.Sequential(  
                        nn.Conv2d(in_channels=1, out_channels=33, kernel_size=(self.char_embedding_dim, 4),  
        ...
        self.fc = nn.Sequential(
                 ...
                 nn.ReLU(),
                 nn.Dropout(0.5),
                 nn.Linear(hidden_dim, self.target_size)    
        )
    ...
    def init_hidden(self, size):
        return (autograd.Variable(torch.zeros(2, 1, size).cuda()), #randn
                autograd.Variable(torch.zeros(2, 1, size).cuda())) #randn
    ...
    def _get_lstm_features(self, sentence, word_seq):
        #[spanlen, step_start, numlabel]
        if len(sentence) != len(word_seq):
            print('something wrong when extracting lstm-cnn features!')
        ...
        for index in range(len(seq)):
            ...
            part_input = char_embeds[start:end,:,:].view(1, 1, self.char_embedding_dim, -1)
            
            two_gram = self.conv2(part_input).view(-1)  
            three_gram = self.conv3(part_input).view(-1)
            four_gram = self.conv4(part_input).view(-1)

            cnn_feature = torch.cat((two_gram, three_gram, four_gram))
            
            char_feats[index, 0, :] = cnn_feature
        
        final_feats = torch.cat((embeds, char_feats), 2) #
    
        lstm_out, _ = self.lstm(final_feats, self.init_hidden(self.hidden_dim//2))
        lstm_out = lstm_out.view(len(sentence), 1, self.hidden_dim) # [seqlen, 1, hidden_size]
        lstm_feats = torch.zeros(self.max_span_len, len(sentence), self.target_size).cuda() #[spanlen, step, numlabel]
        lstm_feats = autograd.Variable(lstm_feats) ####
        for span_len in range(1, self.max_span_len+1):
            for step in range(len(sentence)-span_len+1):
                start = step
                end = step + span_len
               #lstm-feature part
                part_input = lstm_out[start:end,:,:]
                part_out, _ = self.lstm2(part_input, self.init_hidden(self.hidden_dim//2))  #[span_len, 1, hidden] 
                
                            
                part1 = part_out[-1, 0, 0:self.hidden_dim//2]
                part2 = part_out[0, 0, self.hidden_dim//2:self.hidden_dim] 
                hidden = torch.cat([part1, part2]).view(1,-1)  
                
                span_feat = self.fc(hidden)                
                lstm_feats[span_len-1, start] = span_feat
        return lstm_feats  
    ...
    def _score_sentence(self, feats, tags):
        ...
        for i in range(len(tags)-1):
            ...
            start = tags[i][0]
            score = score  + feats[span_len-1,start,self.tag_to_ix[tags[i][2]]] +self.transitions[self.tag_to_ix[tags[i + 1][2]], self.tag_to_ix[tags[i][2]]]
        span_len = tags[-1][1]-tags[-1][0]
        start =tags[-1][0]
        #feat
        score = score +feats[span_len-1,start,self.tag_to_ix[tags[-1][2]]] + self.transitions_bound[0, self.tag_to_ix[tags[0][2]]] + self.transitions_bound[1, self.tag_to_ix[tags[-1][2]]]
        return score
    ...
    def _viterbi_decode(self, feats, sentence):
        ...
        for i in range(len(sentence)):
            for next_tag in range(self.target_size):
                ...
                if next_tag == self.tag_to_ix["O"]:
                    if i == 0:
                        score = feats[0, 0, next_tag]+self.transitions_bound[0, next_tag]
                    ...
                    for pre_tag in range(self.target_size):
                        score = feats[0, i, next_tag] + forward_var[pre_tag, i-1] + self.transitions[next_tag, pre_tag] 
                ...
                for j in range(1, self.max_span_len+1):
                    if i-j == -1:
                        score = feats[j-1, 0, next_tag]+self.transitions_bound[0, next_tag]
                        temp.append((-1, i, -1, next_tag, score))
                        break
                    for pre_tag in range(self.target_size):
                        score = feats[j-1, i-j+1, next_tag] + forward_var[pre_tag, i-j]+self.transitions[next_tag, pre_tag] 
        ...
        score = terminal_record+self.transitions_bound[1].data
        value, [index] = torch.max(score, 0)
        step = len(sentence)-1
        r_result = [] 
        while True:
    ...
    def neg_log_likelihood(self, sentence, tags, word_seq):
        seqlen = len(sentence)
        feats = self._get_lstm_features(sentence, word_seq)
    ...
    def forward(self, sentence, word_seq):  # dont confuse this with _forward_alg above.
        ...
        lstm_feats = self._get_lstm_features(sentence, word_seq)

        # Find the best path, given the features.
        tag_seq = self._viterbi_decode(lstm_feats, sentence)
        return tag_seq
...
model = Atten_SRNN_semiCRF(len(word_to_ix), tag_to_ix, EMBEDDING_DIM, HIDDEN_DIM, max_span_len, word_to_ix, word_vecs, char_to_ix, char_vecs, CHAR_EMBEDDING_DIM)
...
for epoch in range(1, epoch_num+1):  # again, normally you would NOT do 300 epochs, it is toy data
    ...
    if lr*0.75**(epoch-1) >= 0.0001:
        print('Learning Rate: '+str(lr*0.75**(epoch-1)))
        optimizer = optim.SGD(model.parameters(), lr=lr*0.75**(epoch-1), momentum=0.9)
    else:
